...
export function QualityScoreCard({
  initialIndex = 0,
  notchText: initialNotchText = "Exterior",
}: QualityScoreCardProps) {
  const [currentIndex, setCurrentIndex] = useState(initialIndex);
  const [direction, setDirection] = useState<"left" | "right" | null>(null);
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const [currentCase, setCurrentCase] = useState<CaseConfig>(
    getCaseConfig(initialNotchText)
  );
  ...
  // Function to get case configuration based on notchText
  function getCaseConfig(notchText: string): CaseConfig {
    switch (notchText) {
      case "Exterior":
        return {
          notchText: "Exterior",
          ...
          scoreDetails: {
            score: 5,
            title: "Good Exterior Condition",
            description:
              "The exterior shows minor wear but has well-maintained landscaping and no major structural issues.",
          },
        };
      case "Interior":
        return {
          ...
          scoreDetails: {
            ...
            title: "Premium",
            description:
              "Well-maintained with recent upgrades and minimal wear.",
          },
        };
      case "Kitchen":
        return {
          notchText: "Kitchen",
          images: [Kitc1, Kitc2, Kitc3],
          currentScore: 6,
          scoreDetails: {
            score: 6,
            title: "Top-tier",
            description:
              "Brand-new, flawless condition with luxury features.",
          },
        };
      case "Bedroom":
        return {
      ...
      default:
        return {
          notchText: "Exterior",
          images: [Ext1, Ext2, Ext3], // Fallback to initial images
          currentScore: 5,
          scoreDetails: {
            score: 5,
            title: "Luxe",
            description: "No specific data available for this section.",
          },
        };
    }
  }

  // Update case when notchText changes (e.g., via props or user action)
  useEffect(() => {
    console.log("QualityScoreCard notchText updated to:", initialNotchText);
    setCurrentCase(getCaseConfig(initialNotchText));
    setCurrentIndex(0);
  }, [initialNotchText]);

  const handlePrevious = () => {
    setDirection("right");
    setCurrentIndex((prev) => (prev + 1) % currentCase.images.length);
  ...
  const handleNext = () => {
    ...
    setCurrentIndex(
      (prev) =>
        (prev - 1 + currentCase.images.length) % currentCase.images.length
    );
    setTimeout(() => setDirection(null), 500);
  };
    ...
    <div className="flex flex-col px-4 sm:px-14 py-4 w-full h-full items-center justify-center gap-6">
      ...
      <div className="w-full flex flex-col items-center">
        <div className="flex justify-between items-center w-full max-w-5xl relative">
          ...
          >
            <span className="text-sm textColor text-wrap">
              {qualityScoreText.qualityScoreLabel}
            </span>
                    ...
                    <div key={score} className="flex items-center gap-3">
                      <p
                        className={cn(
                          ...
                          score === currentCase.currentScore &&
                            "border-4 border-[#37D3AE]"
                        )}
                      >
                        {score}
                      </p>
      ...
      <div className="w-full max-w-5xl bg-[#DBE9FF] rounded-[32px] relative">
        ...
        <div className="relative h-[450px] w-full overflow-hidden">
          <div className="h-full w-full p-2 absolute inset-0">
            <img
              src={currentCase.images[currentIndex] || "/placeholder.svg"}
          ...
          <Button
            ...
            disabled={currentCase.images.length <= 1}
          >
        ...
        <div className="w-full max-w-5xl bg-[#DBE9FF] rounded-b-[33px] px-4 pb-3 mt-0">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <p className="font-[ClashDisplay-Semibold] text-lg text-[#0246AC]">
                {currentCase.scoreDetails.title}
              </p>
              <p className="font-[Geologica] font-extralight text-base text-[#0246AC]">
                {currentCase.scoreDetails.description}
              </p>
            </div>
          </div>
        </div>
